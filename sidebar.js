...
(function () {
    ...
    try {
      if (typeof window.saveBeatToFirebase === "function") {
        ...
        window.saveBeatToFirebase({
          ...
          pattern:       beat.patterns,
          pianoRollNotes: beat.pianoRollNotes,
          localId:       beat.id,        // keep local ID for cross-reference
  ...
  function loadCloudBeatIntoSequencer(beat) {
    ...
    const cloudPatterns = beat.pattern || beat.patterns;
  ...
  // ---- Account system ----
  function updateSidebarUsernameDisplay() {
    const usernameEl = document.getElementById("sidebar-username");
    if (!usernameEl) return;
    const user = loadUserFromStorage();
    usernameEl.textContent = user && user.username ? user.username : "Guest";
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Small delay ensures Firebase SDK has had time to init before first fetch.
    setTimeout(() => {
      renderCloudBeatsList();
    }, 1500);
    // ===== End Firebase addition =====
